...
import sys
import logging
from news_vector_filter import NewsVectorFilter
...
class CommonNewsSpider(base_news_spider.BaseNewsSpider):
    ...
    def normal_item_persistence(self, item, task, response):
        ...
        if self.PUT_IN_STORAGE:
            if not storage.url_exists(item.url):
                put_done = True
                if config.CHOSEN_SERVER == 'server1':
                    v = NewsVectorFilter.get_vector(item, True)
                    storage.add_page(raw=item.raw, title=item.title, visited_at=item.fetched_at,
                                     created_at=item.t_stamp,
                                     author=item.author, text=item.content, url=item.url, task_no=item.task_no,
                                     region=v[0], type=v[1], category=v[2], language=v[3], source=v[4])
                elif config.CHOSEN_SERVER == 'server2':
                    storage.add_page(raw=item.raw, title=item.title, category=item.category, crawled_at=item.fetched_at,
                                     created_at=item.t_stamp,
                                     author=item.author, text=item.content, url=item.url, source=item.source,
                                     task_no=item.task_no)
                else:
                    self.logger_con.error("unknown server name: " + config.CHOSEN_SERVER)
                    put_done = False
                if put_done:
    ...
    def normal_item_check(self, item, task, response):
        ...
        if item.t == '':
            ...
            return False
        if item.t_stamp == 0:
            self.logger_con.warning('TIME STAMP ERROR: ' + doc_url)
            self.logger_file.warning('TIME STAMP ERROR: ' + doc_url)
            raise Exception
        if item.title == '':
